...
class BaseDescent:
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        loss = np.dot((y - np.dot(x,self.w)).transpose(), (y - np.dot(x,self.w))) / x.shape[0]
        return float(loss)
...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        gradient = 2 * np.dot(x.transpose(), (-y + np.dot(x, self.w))) / x.shape[0]
        return gradient
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        gradient = 2 * np.dot(x[indexes].transpose(), (-y[indexes] + np.dot(x[indexes], self.w))) / self.batch_size
        return gradient
